...
void main()
{
  ...
  /* generate phosphor mask (1080p) */
  if (mod(int(center.x * texture_dim.x), 3) == 0)
    mask = vec3(1.0, mask_opacity, mask_opacity);
  else if (mod(int(center.x * texture_dim.x), 3) == 1)
    mask = vec3(mask_opacity, 1.0, mask_opacity);
  else
    mask = vec3(mask_opacity, mask_opacity, 1.0);
