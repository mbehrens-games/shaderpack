...
/* NTSC gamma of 2.22, TV gamma of 2.35 - 2.55  */
/* (from www.poynton.com/PDFs/GammaFAQ.pdf)     */
...
void main()
{
  ...
  /* apply brightness, contrast, hue, and saturation settings */
  yiq_color.x = mix(black_level, white_level, yiq_color.x);
  yiq_color.yz = saturation * mat2(cos(hue), -sin(hue), sin(hue), cos(hue)) * yiq_color.yz;

  /* convert from yiq to rgb */
  rgb_color = yiq2rgb_matrix * yiq_color;

  /* gamma correction */
  rgb_color = pow(rgb_color, vec3(NTSC_GAMMA / gamma, NTSC_GAMMA / gamma, NTSC_GAMMA / gamma));
